/***********************************************************
#
# Extended from https://github.com/YevgeniyEngineer/KDTree 
#
************************************************************/
...
template <typename CoordinateType, std::size_t number_of_dimensions> class KDTree final {
  ...
  public:
    ...
    template <typename T>
    explicit KDTree(const T& points, bool threaded = false) : KDTree(points, 0, points.size(), threaded) { }

    template <typename T>
    explicit KDTree(const T& points, size_t start, size_t count, bool threaded = false) : root_(nullptr) {
    ...
    void findNearestNeighbourForEachTarget(const std::vector<PointType> &targets, std::vector<ReturnType> &neighbours, bool threaded = false) const {
        neighbours.clear();
    ...
    /// @param result Result list containing a pair of point index and distance squared to target
    void findKNearestNeighboursWithinRadiusSquared(const PointType &target, std::size_t k, CoordinateType radius_squared, std::vector<ReturnType> &result) const {
        result.clear();
        if (k == 0) return;
    ...
    void findAllNearestNeighboursWithinRadiusSquared(const PointType &target, CoordinateType radius_squared, std::vector<ReturnType> &result, bool sort = true) const noexcept {
        result.clear();
  ...
  private:
    ...
    [[nodiscard]] constexpr inline CoordinateType distanceSquared(const PointType &p1, const PointType &p2) const noexcept {
    ...
    /// @return root node
    [[nodiscard]] Node *buildTreeRecursively(typename std::vector<Node>::iterator begin, typename std::vector<Node>::iterator end, std::size_t index) const noexcept {
        if (begin >= end) {
            return nullptr;
        }
